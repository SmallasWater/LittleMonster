...
public class GameCoreDownload {
    ...
    public static final String MINIMUM_GAME_CORE_VERSION = "1.6.13";
    ...
    private static int checkAndDownload(int retry) {
        ...
        if (plugin != null) {
            if (!VersionUtils.checkMinimumVersion(plugin, ACTUAL_MINIMUM_GAME_CORE_VERSION)) {
                LittleMonsterMainClass.getInstance().getLogger().warning("MemoriesOfTime-GameCore依赖版本太低！正在尝试更新版本...");
                ...
                if (file != null) {
                    ...
                    LittleMonsterMainClass.getInstance().getLogger().error("删除旧版本失败！请手动删除！");
                }
            }
        }

        if (plugin == null || plugin.isDisabled()) {
            LittleMonsterMainClass.getInstance().getLogger().info("尝试从 " + url + " 下载 MemoriesOfTime-GameCore 中...");
            ...
            try {
                ...
                download(url, file, (len, fullLength) -> {
                    ...
                    if (d - last.get() > 15) { // 每15%提示一次
                        LittleMonsterMainClass.getInstance().getLogger().info("已下载：" + d + "%");
                        last.set(d);
                    }
                });
            } catch (Exception e) {
                LittleMonsterMainClass.getInstance().getLogger().error("MemoriesOfTime-GameCore依赖下载失败！");
                return checkAndDownload(++retry);
            }

            LittleMonsterMainClass.getInstance().getLogger().info("MemoriesOfTime-GameCore依赖下载成功！");
